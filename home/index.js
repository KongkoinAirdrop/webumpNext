...
export default function Home() {
        ...
        client.queryContractSmart(getLighthouseContract(config.network), { get_collection: { collection: config.collection_address } }).then((result) => {
            ...
            for (let i = 0; i < config.groups.length; i++) {
                for (let j = 0; j < result.mint_groups.length; j++) {
                    ...
                    let groupConfig = config.groups[i]
    ...
    function checkAddress() {
        ...
        return "sei1w69mqddj48lq2ewgh0e28srsw2fxltjektuc77"
    }
    const refreshMyMintedNfts = async () => {
        ...
        let balance = await client.getBalance(checkAddress(), "usei")
        setBalance(new BigNumber(balance.amount).div(1e6).toString())

        client.queryContractSmart(getLighthouseContract(config.network), { balance_of: { address: checkAddress(), collection: config.collection_address } }).then((result) => {
    ...
    const manageWhitelist = (currentPhase) => {
        if (wallet !== null) {
            if (typeof currentPhase.allowlist !== 'undefined' && currentPhase.allowlist !== null) {
                let allowlist = currentPhase.allowlist.find((a) => a === checkAddress())
                if (allowlist) {
    ...
    const mint = async () => {
        ...
        if (currentPhase.merkle_root !== '' && currentPhase.merkle_root !== null) {
            ...
            merkleProof = tree.getProof(Buffer.from(keccak_256(checkAddress()))).map(element => Array.from(element.data))
            hashedAddress = Array.from(Buffer.from(keccak_256(checkAddress())))
        }
        ...
        try {
            ...
            const mintReceipt = await client.executeMultiple(checkAddress(), instructions, "auto")
            ...
            loadNowMintedMetadata(tokenIds).then((metadata) => {
    ...
    const loadNowMintedMetadata = async (mints) => new Promise(async (resolve, reject) => {
        let metadata = []
        let promises = []
            ...
            <C.Container>
                <C.Header>
                    ...
                    {wallet !== null && (
                        ...
                        >
                            <DropdownItem onClick={() => navigator.clipboard.writeText(checkAddress())}>Copy Address</DropdownItem>
